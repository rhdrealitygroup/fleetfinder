...
- **Status:** Fixed & verified (build-gated, migration applied, advisors clean). Re-verify live after deploy that a 2nd identical decode-vin returns `provider:"cache"`.
...
- **Status:** Fixed & verified (build-gated). Live re-verify after deploy: a live-fallback model's picker names match a snapshotted model's style. Stored rows refresh to the new cleaning on the next nightly `refresh-catalog`.
