...
- **Data remediation (DEFERRED to owner — prod data mutation + per-dealer $0.002 quota):** the code fix only affects FUTURE backfills; the already-truncated rows (e.g. 1026774) keep their 20-make tag until re-probed. Owner should re-run `GET /api/cron/sync-dealers?backfill_makes=1&state=NY&limit=…` (and NJ) — it idempotently overwrites `makes` with the now-complete list. The scheduled sync-dealers cron preserves `makes` (never rewrites them), so the stale tags won't self-heal without this manual re-run. NOT run here per §0.4 (writes shared prod data, spends quota at scale).
- **Status:** Code fixed & build-gated on `audit/v3` (NOT merged). Data remediation deferred to owner (documented above).
