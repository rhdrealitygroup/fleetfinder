...
- **Status:** Fixed & verified — MERGED to main + deployed (v2 `ffcd25a`, ff `d4bb0a1`). Parser proven against the live decode (hvf 0→47, feats 0→249; chips 0/25→18/25). Post-deploy live: `GET https://www.lotcompass.com/` → 200, `POST /api/live-search` (unauth) → 401 JSON (route deployed, gate intact). End-to-end authed feature-filter check on the deployed app still pending the owner's authed session (existing `vin_decode_cache` rows refresh to populated hvf on the 30-day TTL).
...
- **Status:** Done (owner-directed change) — build/deno-gated, MERGED to main + deployed (v2 `ffcd25a`, ff `d4bb0a1`). No Auto.dev API call remains in any functional path; `grep autoDev|adListing|AUTO_DEV src` empty in both repos. Post-deploy live `/api/live-search` (unauth) → 401 JSON (route healthy).

### BUG-0028 — `phraseMatch` throws on a >32,767-char needle → unhandled 500 on the option-filter path
- **Date:** 2026-06-22  **Severity:** Low (robustness / self-inflicted 500)  **Found by:** audit/v3 RUN 4 (Pass E, VP-8 adversarial)
- **Area:** `src/lib/marketcheck.ts#phraseMatch` (LotCompass) + `base44/functions/_shared/mc.ts#phraseMatch` (FleetFinder). Consumed by the `live-search` option filter (`option_query` terms + `option_names`) and `diagnose` (`phraseMatch`/`phraseMatchEither` on user `exterior_color`/`interior_color`/`trim`/`option_names`).
- **Symptom:** an authenticated client that POSTs an oversized single `option_query` token, `option_names[]` entry, or diagnose color/trim value (> 32,767 chars) makes `new RegExp(...)` throw `SyntaxError: Invalid regular expression: too large`. The option-filter block in `live-search` is NOT wrapped in try/catch (it sits after the `searchMarketCheck` try/catch), so the throw propagates out of the handler → an **unhandled 500** instead of the graceful JSON the route should return.
- **Root cause:** `phraseMatch` escapes the needle and builds `(^|[^a-z0-9])${esc}([^a-z0-9]|$)`. V8 caps a regex source at ~2^15 chars; a needle longer than that throws at construction. No upper bound was placed on needle length. (Escaping can double length, so a needle of ~16k metachars triggers it even sooner.) Prior runs' VP-8 only tested a 5,000-char needle (no throw), so the boundary was missed.
- **Pattern:** edge-input robustness on a metered route — an unvalidated user value reaching a regex compiler (§3 VP-7 edge inputs / honest error surfacing). Hunt every `new RegExp(userValue)` for a missing length/try-guard.
- **Fix:** cap the needle at `MAX_NEEDLE=512` in `phraseMatch` (both repos) — return `false` for anything longer. An over-long "phrase" can never legitimately match a real option/feature/color/trim, so `false` is the correct graceful result; `phraseMatchEither` inherits the guard. 512 covers every real value with wide margin (longest real option name ≈ 60 chars), and the escaped pattern stays ≈ ≤1k — far below the engine limit.
- **Evidence (before):** unit (exact `phraseMatch` copy) — needle len 32000 → OK; len **33000 → `SyntaxError: …too large`**; binary-searched threshold = **32,767**. Code: `live-search/route.ts` option block (~251-296) is outside the only try/catch (215-221), so the throw is unhandled.
- **Evidence (after):** unit — len 513 / 16384 (all backslashes) / 33000 / 100000 → **no throw, return false**; real phrases `sun/moonroof`, `premium speakers`, `apple carplay`, and a 59-char real phrase still **MATCH**; `tow` still word-bounded (no "Towel" false-match); at-cap 512 doesn't throw. `npm run build` exit 0 (v2); `deno check` OK on all 8 `_shared/mc.ts` importers (ff).
- **Status:** Fixed & VP-8-survived (unit + build/deno-gated). Branch `audit/v3`, NOT merged. Live re-verify after deploy with an authed session (pre-deploy the deployed app still runs the unguarded version, so the 500 reproduces there until merge).

### BUG-0029 — `backfill_makes` bare `facets=make` truncates at 20 → mega-dealers hidden in the directory for their 21st+ makes
- **Date:** 2026-06-22  **Severity:** Med-Low (live in prod data; affects only dealers selling >20 new makes)  **Found by:** audit/v3 RUN 4 (Pass 2, VP-1 facet-size check)
- **Area:** `src/app/api/cron/sync-dealers/route.ts` — the manual `?backfill_makes` path that tags each `dealer_catalog` row with the makes it sells (consumed by the `dealers/catalog` make filter, BUG-0005). LotCompass-only (FleetFinder has no `dealer_catalog`/makes backfill).
- **Symptom:** a dealer that sells **more than 20 distinct new makes** (large auto groups) is tagged with only its **top 20** makes. The `dealers/catalog` make filter then treats it as a TAGGED dealer, so for any make ranked 21+ it's neither a tagged match nor "untagged-with-inventory" → the dealer is **wrongly excluded** from the directory when a broker filters for a make it actually sells.
- **Root cause (P3 / BUG-0001/0017 class):** the backfill requested `facets=make` with **no explicit size**. MarketCheck's default facet size is **20**, so the response silently caps at the 20 highest-count makes. EVERY other facet call in the codebase specifies an explicit size (`trim|0|100|1`, `exterior_color|0|100|1`, `model|0|250|1`, …) — this lone bare facet was the oversight.
- **Pattern:** a facet/rows size omitted → the API substitutes a smaller default than the code needs → silent tail-drop. Same family as BUG-0001 (rows>50→10) and BUG-0017 (catalog sampler). Hunt every facet/`rows` for a missing explicit size.
- **Evidence (before — LIVE, my key):** full index `facets=make` → **20** items vs `make|0|100|1` → **53**, proving bare defaults to 20. Prod data: `dealer_catalog` id `1026774` (NY, 9,666 listings) stores **20** makes; live `make|0|100|1` for that dealer → **24** makes — the 4 it's MISSING are **Cadillac, Dodge, INFINITI, Lincoln**, so the dealer is currently invisible in the NY directory when filtering for any of those four. (Also confirmed dealer `10020930` sells 22 new makes; bare drops Dodge+RAM.)
- **Fix (code):** `facets=make` → `facets=make|0|100|1` (offset 0, size 100, count-sorted) in the backfill probe. 100 exceeds any real dealer's new-make count (~24 max observed); the makeless-`["—"]` sentinel path is unchanged. Commit: fleetfinder-v2 (this branch).
- **Evidence (after):** the same probe with `make|0|100|1` returns the COMPLETE list for both affected dealers (24 / 22). `npm run build` exit 0.
- **Data remediation (DONE 2026-06-23, owner-authorized):** CORRECTION to the original plan — the `?backfill_makes` endpoint filters to `makes IS NULL OR makes='{}'`, so it only tags UNTAGGED dealers and would NOT re-probe a tagged-but-truncated one. Re-running it alone could not fix the truncated rows. Scoped the real blast radius instead: exactly **ONE** dealer in all of `dealer_catalog` had ≥20 stored makes (the cap signature) — `1026774` "West Herr Auto Group" (NY). Re-probed its complete list live (`make|0|100|1` → 24) and applied a targeted single-row UPDATE: `old_count 20 → new_count 24`, makes added = **Dodge, Lincoln, INFINITI, Cadillac**. Verified: `makes @> {make}` now true for all four, so the dealer surfaces correctly in the NY directory for those makes. No other dealer was affected (a dealer is only truncated if it sells >20 makes → caps at exactly 20; only one such row existed).
- **Status:** Code fixed (`facets=make|0|100|1`) — MERGED to main + deployed (v2 `c43cd4f`). Data remediation DONE (1 dealer corrected + verified live). Closed.

### BUG-0030 — team-route seat sync mutated the Stripe subscription with NO idempotency key (invariant violation, BUG-0018 sibling)
- **Date:** 2026-06-22  **Severity:** Low (invariant consistency — no double-billing was possible)  **Found by:** audit/v3 RUN 5 (Pass 2, money-path read)
- **Area:** `src/app/api/team/route.ts#syncSeatQuantity` — called after an agent is added/removed to reconcile the paid seat quantity onto the org's Stripe subscription. LotCompass-only (FleetFinder billing is a separate Base44 `manage_billing`).
- **Symptom:** none observed in practice. The seat quantity is absolute (`quantity: desired`), `proration_behavior:"none"`, and guarded by `desired === currentQty → return`, so no double-billing was ever possible — but the two `stripe.subscriptions.update(...)` calls carried no `idempotencyKey`, violating the app's "every Stripe sub mutation uses an idempotency key" invariant (CLAUDE.md money-paths).
- **Root cause:** BUG-0018 added an idempotency key to the IDENTICAL seat-quantity true-up in the Stripe webhook, but the team-route sibling (`syncSeatQuantity`) was missed — it's in-process (not webhook-triggered), so the redelivery rationale didn't obviously apply, but the invariant is stated absolutely.
- **Pattern:** money-path idempotency invariant (same as BUG-0018). Hunt every `stripe.*.update/create` for a missing key.
- **Fix:** added `idempotencyKey: seat-sync-${subId}-${currentQty}to${desired}` to BOTH `subscriptions.update` calls — keyed on the FROM→TO transition (not just the target) so a repeated value can't make Stripe REPLAY a stale cached response (the trap the `update-seats` route documents). Commit: fleetfinder-v2 (this branch).
- **Class hunt (the whole repo):** audited every Stripe mutation. All other un-keyed calls are idempotent-by-nature and were accepted by prior audits: `subscriptions.cancel` (canceling is a no-op on a canceled sub), fixed-id `coupons.create` (guarded by retrieve-first), absolute-value discount/price `subscriptions.update` (comp reconcile), `cancel_at_period_end` boolean set (stripe/cancel), ephemeral `billingPortal.sessions.create`, and the intentional un-keyed `prices.create` fresh-mint fallback (when the keyed key replayed an archived price). Already-keyed: webhook seat true-up (BUG-0018), `update-seats`, checkout customer/price/session, referral credits. So `syncSeatQuantity` was the ONLY quantity-class mutation missing a key.
